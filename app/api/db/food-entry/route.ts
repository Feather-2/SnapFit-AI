...
export const POST = withAuth(async (request) => {
  try {
    ...
    // 验证必填字段
    const requiredFields = ['logId', 'foodName', 'consumedGrams', 'mealType', 'nutritionalInfoPer100g', 'totalNutritionalInfoConsumed', 'isEstimated']
  ...
  try {
    ...
    const foodEntries = await prisma.foodEntry.createMany({
      data: entries.map((entry: any) => ({
        ...
        totalNutritionalInfoConsumed: JSON.stringify(entry.totalNutritionalInfoConsumed),
